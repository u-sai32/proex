...
public class Task extends Service
  ...
  public static final String WAKELOCK_TAG = "TaskWakeLock";
  ...
  public void onCreate() {
    ...
    // TaskType is a special Task Property that we need to load early
    // To return the appropriate value from onStartCommand
    $TaskType();
    ...
    // Always Create a Wakelock acquire only if needed
    if (this.wakeLock == null) {
      PowerManager powerManager = (PowerManager) getSystemService(POWER_SERVICE);
      wakeLock = powerManager.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, WAKELOCK_TAG);
      // We are probably acquired many times by the startTask
      wakeLock.setReferenceCounted(false); // so that one release() is enough
    }
   ...
   */
  protected void $TaskType() {    // This must be declared protected because we are called from Task1 which subclasses
   ...
   * respectively.  The actual implementation appears in {@code runtime.scm}.
   */
  protected void $Initialize() {    // This must be declared protected because we are called from ReplTask which
    // subclasses us and isn't in our package.
    throw new UnsupportedOperationException();
  }
