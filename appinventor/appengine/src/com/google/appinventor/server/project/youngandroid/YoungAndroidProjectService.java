...
public final class YoungAndroidProjectService extends CommonProjectService {
  ...
  public long addFile(String userId, long projectId, String fileId) {
    if (fileId.endsWith(FORM_PROPERTIES_EXTENSION) || fileId.endsWith(TASK_PROPERTIES_EXTENSION) ||
      ...
      // If the file to be added is a form file or a blocks file, add a new form file, a new task file, a new
  ...
  public long deleteFile(String userId, long projectId, String fileId) {
    if (fileId.endsWith(FORM_PROPERTIES_EXTENSION) || fileId.endsWith(TASK_PROPERTIES_EXTENSION) ||
        fileId.endsWith(BLOCKLY_SOURCE_EXTENSION)) {
      // If the file to be deleted is a form file or a task file or a blocks file,
      // delete the form file, task file, blocks file and the yail file.
      // Also, if there was a codeblocks file laying around
      ...
      String formFileName = YoungAndroidFormNode.getFormFileId(qualifiedFormName);
      String taskFileName = YoungAndroidTaskNode.getTaskFileId(qualifiedFormName);
      String blocklyFileName = YoungAndroidBlocksNode.getBlocklyFileId(qualifiedFormName);
